...
function check_wind_speed(bodyweight, sail_size, level)
    # df_wind = collect_wind_data("Sankt Peter-Ording")
    df_wind = CSV.read("data/test_wind_data.csv", DataFrame)
    ...
    for i in 1:size(df_wind, 1)
        if df_wind."windspeed_10m"[i] < 4
            push!(results, "Wind is too low for windsurfing, consider waiting for better conditions.")
        elseif df_wind."windspeed_10m"[i] >= 28
            push!(results, "Wind is too strong for windsurfing, consider waiting for better conditions.")
        elseif 4 <= df_wind."windspeed_10m"[i] < lb_wind
            push!(results, "Wind is too low for your sail size, consider using a bigger sail of 
            up to $(ub_sail) m² and check again.")
        elseif ub_wind < df_wind."windspeed_10m"[i] < 28
            push!(results, "Wind is too strong for your sail size, consider using a smaller sail of 
            ...
            if current == "too low for your skill level" || current == "too strong for your skill level"
                push!(results, "Wind is $(current), consider waiting for better conditions.")
            else
                push!(results, "Wind is $(current) for windsurfing.")
            end
        end
    end
    df_results[!, "recommendations"] = results
    return df_results
end
